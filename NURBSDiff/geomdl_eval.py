...
class GeomdlEval(torch.nn.Module):
    """
    ...
    def __init__(self, m, n, dimension=3, p=3, q=3, out_dim_u=32, out_dim_v=128, method='tc', dvc='cpp'):
        super(GeomdlEval, self).__init__()
        self.m = m
        self.n = n
        ...
        self.surf.order_u = p + 1
        self.surf.order_v = q + 1
        self.surf.ctrlpts_size_u = m
        self.surf.ctrlpts_size_v = n
        if self.dvc == 'cuda':
            self.u = self.u.cuda()
            self.v = self.v.cuda()
