...
def nurbs_reconstruction_loss(nu, nv, output, points, config, sqrt=False):
    ...
    output = output.view(config.batch_size, 5, 5, 4)
...
from train_open_spline_utils.src.model_nips_rational import DGCNNControlPoints


config = Config('./train_open_spline_utils/configs/config_nips3_test_rational.yml')

control_decoder = DGCNNControlPoints(5, num_points=10, mode=config.mode)
...
nu, nv = uniform_knot_bspline(5, 5, 3, 3, 40)
...
nurbs_layer = SurfEval(5,5, dimension=3, p=3, q=3, out_dim_u=40, out_dim_v=40, method='tc', dvc='cuda')
nurbs_layer.cuda()
...
for val_b_id in range(config.num_test // config.batch_size - 2):
    ...
    for b in range(config.batch_size):
        ...
        if anisotropic:
            ...
            output[:,:,:-1][b] = output[:,:,:-1][b] * s.reshape(1, 3) / torch.max(s)
    ...
    l_reg, permute_cp = control_points_permute_reg_loss(
        output[:,:,:-1], control_points, 5
    )

    laplac_loss = laplacian_loss(
        output[:,:,:-1].reshape((config.batch_size, 5,5, 3)),
        permute_cp,
        dist_type="l2",
    )
