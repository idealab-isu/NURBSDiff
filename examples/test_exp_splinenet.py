...
from train_open_spline_utils.src.loss import laplacian_loss,chamfer_distance_one_side
from NURBSDiff.surf_eval import SurfEval
from train_open_spline_utils.src.loss import (uniform_knot_bspline,
...
config = Config('./train_open_spline_utils/configs/config_nips_test_splinenet.yml')
...
for val_b_id in range(config.num_test // config.batch_size - 2):
    ...
    if if_optimize:
        ...
        cd, optimized_points = spline_reconstruction_loss_one_sided(nu_3, nv_3, new_points, points, config, sqrt=True)
        optimized_points = optimized_points.data.cpu().numpy()

    l_reg, permute_cp = control_points_permute_reg_loss_splinenet(
        output, control_points, 20
    )
...
print(results)
print(
    " Test CP Loss: {}, Test CD Loss: {},  Test Lap: {}".format(
