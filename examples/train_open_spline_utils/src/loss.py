...
from train_open_spline_utils.src.utils import chamfer_distance, chamfer_distance_one_side
...
def control_points_permute_reg_loss(output, control_points, grid_size):
    ...
    # best with the input.
    return torch.tensor([0]), control_points[np.arange(batch_size), 1]



def control_points_permute_reg_loss_splinenet(output, control_points, grid_size):
    """
    control points prediction with permutation invariant loss
    :param output: output of the network
    :param control_points: N x grid_size x grid_size x 3
