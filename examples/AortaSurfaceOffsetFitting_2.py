...
def main():
    ...
    AortaSurfaces = exchange.import_smesh('D:/ResearchDataCodes/NURBS_IGA/Aorta_2/')
    ...
    for i in range(len(AortaSurfaces)):
        ...
        EdgePtsIdx = cpu.EdgeCtrlPtsIndexing(AortaSurfaces[i], EdgePtsCount)
        AortaEdgePtsIdx.append(EdgePtsIdx)


    # AortaEdgePtsIdx = cpu.MapEdgeCtrlPts(AortaSurfaces, AortaEdgePtsIdx)
    pass
    patches = 18
    ...
    SurfPtIdx = np.zeros([patches, evalPtsSize * evalPtsSize], dtype=np.bool)
        ...
        for i in range(0, evalPtsSize):
            ...
            for j in range(0, evalPtsSize):
                ...
                if (u or v == 0.0) or (u or v == 1.0):
                    SurfPtIdx[patch, i * evalPtsSize + j] = True

    cpu.SurfEdgePtsMapping(AortaSurfaces, SurfPtIdx, delta)
