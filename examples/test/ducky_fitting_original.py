...
from geomdlTorch import utilities
...
def main():
    ...
    if case=='Ducky':
        ...
        ctrlpts = np.array(exchange.import_txt("../Ducky/duck1.ctrlpts", separator=" ")).reshape(num_ctrl_pts1 * num_ctrl_pts2,3)
    ...
    for i in pbar:
        ...
        def closure():
            ...
            close_loss_column = 0.001 * torch.norm(inp_ctrl_pts[:, :, 0, :] - inp_ctrl_pts[:, :, -1, :])
            close_loss_row = 0.001 * torch.norm(inp_ctrl_pts[:, 0, :, :] - inp_ctrl_pts[:, -1, :, :])
            ...
            loss = ((target-out)**2).mean() + close_loss_column + close_loss_row
    ...
    # save the predicted control points in a .ctrlpts file
    with open("generated/duck1_reparam.ctrlpts", "w") as f:
        inp_ctrl_pts = inp_ctrl_pts.detach().cpu().numpy().squeeze().reshape(num_ctrl_pts1, num_ctrl_pts2, 3)
        for i in range(num_ctrl_pts1):
            for j in range(num_ctrl_pts2):
                line = str(inp_ctrl_pts[i, j, 0]) + ' ' + str(inp_ctrl_pts[i, j, 1]) + ' ' + str(inp_ctrl_pts[i, j, 2]) + '\n'
                f.write(line)
    # save the predicted output to a .off file
    with open("generated/duck1_reparam.off", "w") as f:
        f.write('OFF\n')
        f.write(str(num_eval_pts_u*num_eval_pts_v) + ' ' + str(0) + ' ' + str(0) + '\n')
