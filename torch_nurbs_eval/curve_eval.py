...
import torch
import numpy as np
from curve_eval_cpp import forward as cpp_forward, backward as cpp_backward
from curve_eval_cuda import pre_compute_basis, forward, backward
import time
...
class CurveEval(torch.nn.Module):
    ...
    def __init__(self, m, dimension=3, p=2, out_dim=32):
        ...
        self.u = torch.linspace(0.0, 0.9999, steps=out_dim,dtype=torch.float32,device='cuda')
...
class CurveEvalFunc(torch.autograd.Function):
    ...
    def forward(ctx, ctrl_pts, uspan, Nu, u, m, p, _dimension):
        ...
        curves_cpp = cpp_forward(ctrl_pts.cpu(), uspan.cpu(), Nu.cpu(), u.cpu(), m, p, _dimension)
        ctx.curves = curves


        
        
        print("Curve comparison mse loss",torch.nn.functional.mse_loss(curves.cpu(),curves_cpp))
    ...
    def backward(ctx, grad_output):
        ...
        # print(grad_output)
        grad_cw = torch.zeros((grad_output.size(0),grad_output.size(1),_dimension+1),dtype=torch.float32,device='cuda')
...
def main():
    ...
    inp_ctrl_pts[0,-1,:2] += 10*torch.rand(2,dtype=torch.float32,device='cuda')
    inp_ctrl_pts[0,2,:2] += 10*torch.rand(2,dtype=torch.float32,device='cuda')
    inp_ctrl_pts[0,-3,:2] += 10*torch.rand(2,dtype=torch.float32,device='cuda')
